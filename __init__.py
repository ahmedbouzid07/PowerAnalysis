...
def encrypt(target, plaintext: bytearray, key: Optional[bytearray]=None, ack: bool=True) -> Optional[bytearray]:
    """Perform encryption on the CW305 FPGA target.

    Args:
        target: CW305 target object to read/write text from.
        plaintext (bytearray): Plaintext to send to the target.
        key (bytearray, optional): Key to send to the target. Defaults to None.
        ack (bool, optional): Check for ack when reading response from the target. Defaults to True.

    Returns:
        bytearray: The encrypted ciphertext.
    """
    # Set the encryption key
    if key:
        target.set_key(key, ack=ack)

    # Write the plaintext to the target
    target.simpleserial_write('p', plaintext)
    ...
    # Read the ciphertext from the target
